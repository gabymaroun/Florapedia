    ...
    <script>
        ...
        <%}%>
            ...
            document.getElementById("Edit").addEventListener("click", function () {
                ...
                document.getElementById("addTaxon").removeAttribute("disabled");
                document.getElementById("Relation").removeAttribute("disabled");
                document.getElementById("DeleteRelation").removeAttribute("disabled");
                document.getElementById("Save").removeAttribute("disabled");
                document.getElementById("Rearrange").removeAttribute("disabled");

            });
            document.getElementById("TaxonHeader").innerHTML = "Nom du " + '<%=Taxon%>';
            document.getElementById("addTaxon").innerHTML = "Ajouter " + '<%=Taxon%>';
            document.getElementById("Ajouter_Taxon").innerHTML = "Ajouter " + '<%=Taxon%>';
            let timerId;
            var MouseX;
            var MouseY;
            var eps = 0.1;
            ...
            function DrawRelations() {
                ...
                Relations.forEach(function (i, j) {
                    ...
                    Mcos = 10 * Math.cos(angle + Math.PI / 7);
                    Msin = 10 * Math.sin(angle + Math.PI / 7);
                    Msinn = 10 * Math.sin(angle - Math.PI / 7);
                    Mcosn = 10 * Math.cos(angle - Math.PI / 7);
            ...
            document.getElementById("DeleteDiv").addEventListener("mouseout", function (e) {
                var div = $("#DeleteDiv");
                var height = $("#DeleteDiv").height(),
                        width = $("#DeleteDiv").width();
                Target(e);

                var top = document.getElementById("DeleteDiv").style.top;
                var left = document.getElementById("DeleteDiv").style.left;
        ...
        <% if (RootId != null) {%>
                ...
                URL += "&dGn=" + DeletedGraph.length;

                DeletedGraph.forEach(function (i, j) {
                    URL += "&dgid" + j + "=" + i.realID;
                });
                ...
                if (Keys[this.value].Key.realID !==<%=C.getRoot().getId()%>) {
                    ...
                    if (Delete === 1) {
                        ...
                        if (Keys[this.value].Key.Type.localeCompare('<%=Taxon%>') !== 0) {
                            if (A.localeCompare("") === 0)
                                DeletedKeys.push({realID: Keys[this.value].Key.realID});
                            Keys.splice(this.value, 1);
                        } else {
                            if (A.localeCompare("") === 0) {
                                var URL = "ModGraphServlet?uc=getgraph&idroot=" + Keys[this.value].Key.realID;
                                $.get(URL, function (responseText) {
                                    if (responseText.localeCompare("1") === 0) {
                                        DeletedGraph.push({realID: Keys[Pos].Key.realID});
                                        Keys.splice(Pos, 1);
                                    } else {
                                        swal({title: "Opération impossible",
                                            text: "Le taxon '" + Keys[Pos].Key.Description + "' possède des clés de determinations",
                                            icon: "warning",
                                            buttons: "Ok",
                                        });
                                    }
                        ...
                        swal({title: "Opération impossible",
                            text: "Le Clé '" + Keys[this.value].Key.Description + "' possède des relations",
                            icon: "warning",
                            buttons: "Ok",
                        });
            ...
            document.getElementById("canvas").addEventListener('contextmenu', event => {
                ...
                Keys.some(function (i, j) {
                    if (i.Key.x < MouseX && i.Key.x + 150 > MouseX && i.Key.y < MouseY && i.Key.y + 50 > MouseY && i.Key.Description.localeCompare("") === 0 || i.Key.x < MouseX && i.Key.x + Paint.measureText(i.Key.Description).width > MouseX && i.Key.y < MouseY && i.Key.y + getTextHeight().height > MouseY && i.Key.Description.localeCompare("") !== 0) {
                        ...
                        if (i.Key.Type.localeCompare('<%=TypesNames.TRANSITION%>') !== 0 && i.Key.Type.localeCompare('<%=TypesNames.ESPECE%>')) {
                            var A = i.Key.realID + "";
                            if (A.localeCompare("") !== 0) {
                                link.setAttribute("href", "ManageGraphs.jsp?Rootid=" + A);
            ...
            c.addEventListener("mousedown", function (event) {
                ...
                if (event.which === 1) {
                    ...
                    if (State.localeCompare("Move") === 0) {
                        ...
                        Keys.some(function (i, j) {
                            if (i.Key.x < MouseX && i.Key.x + i.Key.width > MouseX && i.Key.y < MouseY && i.Key.y + getTextHeight().height > MouseY) {
                                if (SelectedKey1 === 9999)
                                    if (i.Key.Type.localeCompare('<%=Taxon%>') !== 0)
                                        ...
                                        swal({
                                            ...
                                            text: "Tu ne peut pas crée une relation pour un Taxon",
                                            icon: "warning",
                                            buttons: "Ok",
                                        });
                                else
                            ...
                            if (SelectedKey1 !== SelectedKey2) {
                                ...
                                if (State.localeCompare("Relation") === 0) {
                                    if (Keys[SelectedKey1].Key.Lvl < Keys[SelectedKey2].Key.Lvl) {
                                        ...
                                        swal({
                                            title: "Opération Impossible",
                                            text: "Tu ne peut pas crée une relation entre clé de même niveau",
                                            icon: "warning",
                                            buttons: "Ok",
                                        });
                                        ...
                                        swal({
                                            title: "Opération Impossible",
                                            text: "Tu ne peut pas crée une relation inverse",
                                            icon: "warning",
                                            buttons: "Ok",
                                        });
                                    }
                                    ...
                                    Relations.some(function (i, j) {
                                        if (i.Relation.Key1 === parseInt(A, 10) && i.Relation.Key2 === parseInt(B, 10)) {
                                            ...
                                            if (canDelete === 1) {
                                                ...
                                                swal({
                                                    title: "Opération Impossible",
                                                    text: "Le clé '" + Keys[i.Key2Pos].Key.Description + "' possède des relations",
                                                    icon: "warning",
                                                    buttons: "Ok",
                                                });
        ...
        <% }
        %>
                    NewKey.Description = '<%=K.getName().toString().replace("&#39", "\\'")%>';
...
<body id="body" onload="Init()" class="" style="background-image:url(image/background.png)">
        ...
        <div class="modal-dialog">
            ...
            <div class="modal-content">
                <div class="modal-header">
                    <h1 class="modal-title">Clé Information</h1>
                </div>
        ...
        <div class="modal-dialog">
            ...
            <div class="modal-content">
                ...
                <div class="modal-footer">
                    ...
                    <button type="button" class="btn btn-default" data-dismiss="modal" id="CloseTax">Fermer</button>
                </div>
            </div>
    ...
    <div class="content">
        <nav class="navbar navbar-inverse" style="background-color: #746999">

            <div class="container-fluid" id="navbarNav">
                <ul class="nav navbar-nav">
                    <li><button id="add" class="btn btn-primary btn-blue" data-toggle="modal" data-target="#myModal" disabled="true" style="background-color: #584179">Ajouter caractère</button></li>
                    <li><button id="addTaxon" class="btn btn-primary btn-blue" data-toggle="modal" data-target="#MyTaxonModel" disabled="true" style="background-color: #584179">Ajouter</button></li>
                    <li><button id="Move" class="btn btn-primary" style="background-color: #584179">Déplacer</button></li>
                    <li><button id="Relation" class="btn btn-primary" disabled="true" style="background-color: #584179">Créer une relation</button></li>
                    <li><button id="DeleteRelation" class="btn btn-primary" disabled="true" style="background-color: #584179">Supprimer une Relation</button></li>
                    <li><button id="Save" class="btn btn-primary" disabled="true" style="background-color: #584179">Sauvegarder</button></li>
                    <li><button id="Rearrange" class="btn btn-primary" disabled="true" style="background-color: #584179">Réarranger</button></li>
                    <li><button id="Edit" class="btn btn-primary" style="background-color: #584179">Modifier</button></li>
                    <li><a href="ManageGraphs.jsp?Rootid=1">Clé des Groupes</a></li>
                </ul>
            </div>
        </nav>
    </div>
    ...
    <div id="DeleteDiv" class="big" >
        <button class="btn btn-primary" id="DeleteKey" >Supprimer</button>
        <div id="TaxonLink"  display="none" >
            <button  class="btn btn-primary"><a href="#" id="link">Montrer le graphe</a></button>
                    </div>
                    </div>
